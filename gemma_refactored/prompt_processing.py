    ...
    for category, subcategories in prompt_groups.items():
        if isinstance(subcategories, dict):
            ...
            for prompt in subcategories:
                all_prompts.append(prompt)
                prompt_info.append(
                    {"prompt": prompt, "category": category, "subcategory": "default"}
                )
...
def save_results(
    ...
    output_dir: str,
    prompt_info: Optional[List[Dict[str, Any]]] = None,
) -> None:
    ...
    summary = {
        ...
        "average_tokens_per_prompt": (
            sum(r.get("num_tokens", 0) for r in results) / len(results)
            if results
            else 0
        ),
    }
        ...
        try:
            ...
            if verbose:
                for i, result in enumerate(batch_results):
                    print(
                        f"  Result {batch_idx + i + 1}: {result['generated_text'][:100]}..."
                    )
    ...
    for category, subcategories in prompt_groups.items():
        ...
        if isinstance(subcategories, dict):
            for subcategory, prompts in subcategories.items():
                ...
                if save_activations_dir:
                    group_activations_dir = (
                        f"{save_activations_dir}/{category}/{subcategory}"
                    )
...
def create_sample_prompts_file(output_path: str) -> None:
    ...
    try:
        ...
        print(
            "  - Subcategories (e.g., 'knowledge', 'creativity', 'python', 'algorithms')"
        )
