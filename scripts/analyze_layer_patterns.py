...
def analyze_layer_patterns(
    ...
    num_layers, expected_hidden_dim = load_model_config_from_activations(
        activations_dir
    )
    ...
    print(
        f"Natural prompts: {len(natural_indices)}, Artifact prompts: {len(artifact_indices)}"
    )
    print("=" * 60)
    ...
    sorted_layers = sorted(
        layer_scores.items(), key=lambda x: x[1]["cohens_d"], reverse=True
    )

    top_layers = []
    for layer, scores in sorted_layers[:10]:
        print(
            f"Layer {layer:2d}: d={scores['cohens_d']:6.3f}, "
    ...
    bars = plt.bar(
        layers, cohens_ds, color=["red" if l in top_layers else "blue" for l in layers]
    )
    plt.xlabel("Layer")
    ...
    plt.bar(
        layers, accuracies, color=["red" if l in top_layers else "blue" for l in layers]
    )
    plt.xlabel("Layer")
    ...
    for layer, scores in layer_scores.items():
        ...
        plt.scatter(
            scores["cohens_d"], scores["accuracy"], s=50, color=color, alpha=0.7
        )
...
def compute_multi_layer_directions(
    ...
    print(
        f"Using {len(natural_indices)} natural and {len(artifact_indices)} artifact prompts"
    )
    ...
    np.savez(output_npz, **{f"layer_{l}": d for l, d in directions.items()})
    ...
    output_meta = Path(
        "saved_models/suppression_vectors/multi_layer_directions_metadata.json"
    )
