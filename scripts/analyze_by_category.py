...
RAW_ACTIVATIONS_DIR = Path(
    os.environ.get("ACTIVATIONS_DIR", "./collected_activations_no_rep")
)
                    ...
                    for global_idx in prompt_indices:
                        ...
                        if batch_offset <= global_idx < batch_offset + batch_size:
                            ...
                            if batch_data.ndim == 3 and local_idx < batch_data.shape[0]:
                                ...
                                batch_data.ndim == 2 and local_idx < batch_data.shape[0]
                            ):
...
def compute_category_metrics(
    natural_activations: np.ndarray, artifact_activations: np.ndarray
) -> Dict[str, float]:
...
def main():
    ...
    batch_dirs = sorted(
        [
            d
            for d in RAW_ACTIVATIONS_DIR.iterdir()
            if d.is_dir() and d.name.startswith("batch_")
        ]
    )
    if not batch_dirs:
        ...
        for category, indices_by_type in categories.items():
            ...
            if natural_acts and artifact_acts:
                ...
                if metrics:
                    ...
                    print(
                        f"{display_name:25} | d={metrics['cohens_d']:5.3f} | "
...
def create_visualizations(results_by_layer: Dict, category_peaks: Dict):
    ...
    categories_sorted = sorted(
        category_peaks.items(), key=lambda x: x[1]["cohens_d"], reverse=True
    )
