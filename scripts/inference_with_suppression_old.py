...
class MLPWrapper:
    ...
    def __call__(self, x):
        ...
        if self.scale > 0:
            ...
            if (
                self.suppressor.intervention_count <= 5
                or self.suppressor.intervention_count % 10 == 0
            ):
                print(
                    f"✓ Applied suppression #{self.suppressor.intervention_count} at layer {self.layer_idx}"
                )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="mlx-community/gemma-3-1b-it-qat-4bit"
    )
    parser.add_argument(
        "--activations-dir", type=str, default="./collected_activations_no_rep"
    )
    parser.add_argument(
        "--categories",
        type=str,
    ...
    if not activations_path.exists():
        ...
        print(
            "  3. ACTIVATIONS_DIR=./collected_activations_no_rep python analyze_by_category.py"
        )
        return
    ...
    if args.compare:
        ...
        generated_tokens = []
        eos_token_ids = (
            {tokenizer.eos_token_id} if hasattr(tokenizer, "eos_token_id") else set()
        )
